...
public class SelectionBuilder {
    ...
    public Cursor query(SQLiteDatabase db, boolean distinct, String[] columns, String orderBy,
        ...
        if (columns != null) {
            ...
            if ((null != mProjectionMap) && mProjectionMap.size() != 0) {
                String[] projectionColumns = mProjectionMap.values().toArray(new String[mProjectionMap.size()]);
                Log.v(TAG, "query(columns=" + Arrays.toString(projectionColumns)
                        + ", distinct=" + distinct + ") " + this);
                return db.query(distinct, mTable, projectionColumns, getSelection(), getSelectionArgs(), mGroupBy,
                        mHaving, orderBy, limit);
            } else {
                Log.v(TAG, "query(columns=" + Arrays.toString(columns)
                        + ", distinct=" + distinct + ") " + this);
